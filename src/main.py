...
# Parameters
Ns = [8, 16, 32]
ns_TRAIN = [80, 160, 320, 640, 1280, 2560, 5120]
ms = [2**6, 2**10, 2**12]
GAMMA = 0.8
n_TEST = 1000
NUM_EXP = 8
...
def train(model, optimizer, train_dataset, N, n_TRAIN, m, exp):
    ...
    fig_dir = os.path.join(script_dir, "../output/training_curves/N=%d/m=%d/n_TRAIN=%d/" % (N, m, n_TRAIN))
...
figure = matplotlib.figure.Figure()
grid_spec = figure.add_gridspec(nrows=len(Ns), ncols=len(ms))

for row, N in enumerate(Ns):
    for col, m in enumerate(ms):
        NTK_loss = numpy.empty([NUM_EXP, len(ns_TRAIN)])
        nn_loss = numpy.empty([NUM_EXP, len(ns_TRAIN)])

        for exp in range(NUM_EXP):
            for n_TRAIN_index, n_TRAIN in enumerate(ns_TRAIN):
                ...
                print(f'Training NTK, N={N}, m={m}, exp={exp}, n_TRAIN={n_TRAIN}')
                NTK.fit(train_inputs.cpu().numpy(), train_targets.cpu().numpy()) # .numpy() only takes tensor in CPU
                print(f'Infering NTK, N={N}, m={m}, exp={exp}, n_TRAIN={n_TRAIN}')
                test_outputs_NTK = NTK.predict(test_inputs.cpu().numpy()) # .numpy() only takes tensor in CPU
                NTK_loss[exp,n_TRAIN_index] = sklearn.metrics.mean_squared_error(test_targets.cpu().numpy(), test_outputs_NTK) # .numpy() only takes tensor in CPU
                ...
                nn_loss[exp,n_TRAIN_index] = get_loss(nn, test_dataset)
        ...
        axes = figure.add_subplot(grid_spec[row,col],title="N=%d, m=%d" % (N, m), xlabel="n_TRAIN", ylabel="l2_loss")
        axes.grid()

        axes.plot(ns_TRAIN, NTK_loss_mean, marker="o", label="NTK")
        axes.fill_between(ns_TRAIN, NTK_loss_mean-NTK_loss_std, NTK_loss_mean+NTK_loss_std, alpha=3/8)
        axes.plot(ns_TRAIN, nn_loss_mean, marker="o", label="NN")
        axes.fill_between(ns_TRAIN, nn_loss_mean-nn_loss_std, nn_loss_mean+nn_loss_std, alpha=3/8)
